...
O'Reilly Japan の電子書籍ページと紙書籍カタログを収集し、一覧しやすいギャラリー形式で公開するプロジェクトです。
...
- 書籍ページ: https://lixxlim.github.io/OReilly-Japan-Ebook-Gallery/book/
...
- 生成ページ: `output/index.html`
- 書籍ページ: `output/book/index.html`
- 画像キャッシュ (ebook): `output/oreilly_ebook_image_cache.json`
- 画像キャッシュ (book): `output/oreilly_catalog_image_cache.json`

生成結果を確認するには、`output/index.html` をブラウザで開いてください。  
（macOS の場合: `open output/index.html`）
...
python3 oreilly_ebook_to_html.py --book-output output/book/index.html
python3 oreilly_ebook_to_html.py --book-cache output/oreilly_catalog_image_cache.json
```
